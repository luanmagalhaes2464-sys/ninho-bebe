...
const defaultItems=[
 ...
 // FRALDAS — alvo da fase; comprar aos poucos porque tamanho e marca variam
 I('d1','Fralda descartável','Fraldas','RN · pacote médio',2,'3º trimestre / chá',true,'Alvo inicial. RN pode ser perdido rapidamente; prefira poucos pacotes e reponha se necessário.','Nascimento'),
 I('d2','Fralda descartável','Fraldas','P · pacote médio',6,'Chá / perto do parto',true,'Alvo da fase, não compra obrigatória de uma vez.','0–3m'),
 I('d3','Fralda descartável','Fraldas','M · pacote médio',10,'Chá / após nascer',true,'Tamanho costuma durar mais; distribuir no chá e comprar por etapas.','3–9m','Esperar'),
 I('d4','Fralda descartável','Fraldas','G · pacote médio',10,'Após 6 meses',true,'Alvo da fase; comprar conforme peso e adaptação à marca.','6–12m','Esperar'),
 I('d5','Fralda descartável','Fraldas','XG · pacote médio',6,'Após 10–12 meses',true,'Alvo inicial da fase; reposição conforme crescimento.','12–24m','Esperar'),
...
function diaperPlan(){const n=Math.max(0,+state.showerGuests||0);return{P:Math.round(n*.20),M:Math.round(n*.38),G:Math.round(n*.29),XG:Math.max(0,n-Math.round(n*.20)-Math.round(n*.38)-Math.round(n*.29))}}
function renderTea(){const p=diaperPlan();$('#view-cha').innerHTML=`<div class="grid-2"><div class="card panel"><div class="section-head"><div><h2>Planejador do chá de fraldas</h2><p>Distribuição prática para reduzir concentração em RN.</p></div></div><div class="field"><label>Número estimado de convidados/presentes</label><input type="number" min="0" value="${state.showerGuests}" onchange="setGuests(this.value)"></div><div class="diaper-plan" style="margin-top:16px"><div class="diaper"><span>Tamanho</span><b>P</b><span>${p.P} presentes</span></div><div class="diaper"><span>Tamanho</span><b>M</b><span>${p.M} presentes</span></div><div class="diaper"><span>Tamanho</span><b>G</b><span>${p.G} presentes</span></div><div class="diaper"><span>Tamanho</span><b>XG</b><span>${p.XG} presentes</span></div></div><div class="notice" style="margin-top:14px">RN continua fora da divisão-padrão. Se quiserem pedir, usem poucos pacotes e ajustem à faixa de peso da marca.</div></div><div class="card panel"><div class="baby-card-illustration">${icons.gift}<span>chá do<br><b>Ian ou Luísa</b></span></div><h2>Lista que conversa com o inventário</h2><p class="body-copy">Depois do chá, basta marcar os itens como <b>Ganhou</b> na guia Enxoval. O Ninho recalcula automaticamente o que ainda falta.</p><button class="btn soft" onclick="go('enxoval')">Abrir inventário</button></div></div>`}
window.setGuests=v=>{state.showerGuests=+v||0;save()};

function renderPreg(){$('#view-gestacao').innerHTML=`<div class="grid-2"><div class="card panel"><div class="section-head"><div><h2>Resumo da gestação</h2><p>Datação configurada pela transferência de embrião D5.</p></div></div><div class="profile-list"><div class="profile-row"><span>Gestante</span><b>${PROFILE.mother}</b></div><div class="profile-row"><span>Idade gestacional hoje</span><b>${weeks} semanas${days?` e ${days} dias`:''}</b></div><div class="profile-row"><span>Transferência FIV</span><b>31/07/2026 · D5</b></div><div class="profile-row"><span>Previsão do parto</span><b>18/04/2027</b></div><div class="profile-row"><span>Bebê</span><b>${PROFILE.babyNames}</b></div><div class="profile-row"><span>Plano</span><b>Agros · Viçosa/MG</b></div></div></div><div class="card panel baby-note"><div class="baby-note-icon">${icons.baby}</div><h2>8 semanas hoje</h2><p>Em 06/09/2026, a transferência D5 de 31/07/2026 corresponde exatamente a 8 semanas de idade gestacional. O app calcula isso automaticamente pela data.</p></div></div><div class="grid-2" style="margin-top:16px"><div class="card panel"><div class="section-head"><div><h2>Rotina do pré-natal</h2><p>Referência do Ministério da Saúde para risco habitual.</p></div><button class="btn soft" onclick="generatePrenatal()">Gerar agenda</button></div><div class="timeline"><div class="timeline-item"><div class="timebadge">≤12</div><div><b>Início do pré-natal</b><span>Preferencialmente até a 12ª semana.</span></div><span class="tag green">agora</span></div><div class="timeline-item"><div class="timebadge">≤28</div><div><b>Mensal</b><span>Consultas mensais até a 28ª semana.</span></div><span class="tag">rotina</span></div><div class="timeline-item"><div class="timebadge">28–36</div><div><b>Quinzenal</b><span>Consultas a cada 2 semanas.</span></div><span class="tag">rotina</span></div><div class="timeline-item"><div class="timebadge">36+</div><div><b>Semanal</b><span>Consultas semanais até o parto.</span></div><span class="tag">rotina</span></div></div></div><div class="card panel"><div class="section-head"><div><h2>Agros · Nascer Saudável</h2><p>Vale conferir antes de comprar tudo.</p></div></div>${vaccine('Curso de pré-natal em Viçosa','Encontros para gestantes sobre gravidez, parto, recém-nascido e aleitamento.')}${vaccine('Visita após o nascimento','O Agros informa visita de enfermeira especializada nas cidades da área de abrangência.')}${vaccine('Kit de cuidados do bebê','Benefício informado às beneficiárias; confirmar retirada/agendamento com o Agros.')}<a class="btn soft link-btn" target="_blank" href="https://www.agros.org.br/saude/programas-e-campanhas-de-promocao-da-saude/nascer-saudavel">Abrir Agros</a></div></div>`}
function vaccine(t,b){return `<div class="vaccine"><div class="vaccine-check">${icons.shield}</div><div><b>${t}</b><p>${b}</p></div></div>`}

function renderMotherVax(){const done=motherVaccines.filter(v=>state.motherVax[v.id]?.done).length;$('#view-vacmae').innerHTML=`<div class="grid-2"><div class="card panel"><div class="section-head"><div><h2>Carteira da Isabela</h2><p>${done} de ${motherVaccines.length} registros marcados.</p></div></div><div class="vax-progress"><span style="width:${Math.round(done/motherVaccines.length*100)}%"></span></div><div class="notice" style="margin-top:14px">Calendário-base do Ministério da Saúde 2026. O histórico vacinal individual pode mudar o que precisa ser feito agora.</div></div><div class="card panel baby-note"><div class="baby-note-icon">${icons.syringe}</div><h2>Próximo marco</h2><p>${weeks<20?`dTpa a partir da 20ª semana — faltam aproximadamente ${20-weeks} semanas.`:weeks<28?'dTpa já está na janela indicada; confirme com o pré-natal.':'dTpa e VSR já estão em janela gestacional; confirme o registro com a equipe.'}</p></div></div><div class="card panel" style="margin-top:16px"><div class="vax-list">${motherVaccines.map(v=>vaxRow(v,'motherVax')).join('')}</div><div class="source"><b>Fonte oficial</b><p>Calendário Nacional de Vacinação — Gestante, Ministério da Saúde, atualizado em 2026.</p><a target="_blank" href="https://www.gov.br/saude/pt-br/vacinacao/calendario">Abrir calendário</a></div></div>`}
function renderBabyVax(){const done=babyVaccines.filter(v=>state.babyVax[v.id]?.done).length;const grouped=[...new Set(babyVaccines.map(v=>v.age))];$('#view-vacbebe').innerHTML=`<div class="grid-2"><div class="card panel"><div class="section-head"><div><h2>Vacinação de Ian ou Luísa</h2><p>Do nascimento até 24 meses.</p></div></div><div class="vax-progress"><span style="width:${Math.round(done/babyVaccines.length*100)}%"></span></div><div class="notice" style="margin-top:14px">O bebê deve nascer em 2027. Esta guia usa o Calendário Nacional 2026 como base e precisa ser revalidada após o nascimento caso o PNI seja atualizado.</div></div><div class="card panel"><div class="baby-card-illustration">${icons.shield}<span>vacinas do<br><b>Ian ou Luísa</b></span></div><p class="body-copy">Ao nascer: hepatite B e BCG. Depois, o app organiza as próximas doses por idade e permite registrar data, local e observações.</p></div></div><div class="vax-groups">${grouped.map(age=>`<div class="card panel"><div class="section-head"><div><h2>${age}</h2><p>${babyVaccines.filter(v=>v.age===age).length} item(ns)</p></div></div>${babyVaccines.filter(v=>v.age===age).map(v=>vaxRow(v,'babyVax')).join('')}</div>`).join('')}</div><div class="card panel" style="margin-top:16px"><div class="source"><b>Fonte oficial</b><p>Calendário Nacional de Vacinação da Criança — Ministério da Saúde, atualização 2026.</p><a target="_blank" href="https://www.gov.br/saude/pt-br/vacinacao/calendario">Abrir calendário</a></div></div>`}
function vaxRow(v,key){const r=state[key][v.id]||{};return `<div class="vax-row ${r.done?'done':''}"><button class="vax-toggle" onclick="toggleVax('${key}','${v.id}')">${r.done?'✓':''}</button><div class="vax-main"><b>${v.name}</b><span>${v.when||v.age} · ${v.dose}</span>${v.note?`<small>${v.note}</small>`:''}${r.date||r.place||r.note?`<div class="vax-meta">${r.date?`<span>📅 ${fmtDate(r.date)}</span>`:''}${r.place?`<span>📍 ${esc(r.place)}</span>`:''}${r.note?`<span>📝 ${esc(r.note)}</span>`:''}</div>`:''}</div><button class="btn tiny" onclick="editVax('${key}','${v.id}')">Registrar</button></div>`}
