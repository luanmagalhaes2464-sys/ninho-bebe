...
const defaultItems=[
 ...
 I('d2','Fralda descartável','Fraldas','P · pacote',12,'Chá / perto do parto',true,'Meta sincronizada com o Chá. Referência Huggies: em torno de 6 fraldas/dia entre 2 e 4 meses; ajuste pelo peso e pela marca.','0–4m'),
 I('d3','Fralda descartável','Fraldas','M · pacote',20,'Chá / após nascer',true,'Meta sincronizada com o Chá. Referência Huggies: cerca de 5 fraldas/dia entre 5 e 10 meses.','5–10m','Esperar'),
 I('d4','Fralda descartável','Fraldas','G · pacote',33,'Chá / após nascer',true,'Meta sincronizada com o Chá. Referência Huggies: cerca de 5 fraldas/dia entre 11 e 20 meses; por isso G recebe a maior fatia.','11–20m','Esperar'),
 I('d5','Fralda descartável','Fraldas','XG · pacote',10,'Chá / após nascer',true,'Meta sincronizada com o Chá. Referência Huggies: cerca de 4 fraldas/dia entre 21 e 26 meses.','21–24m','Esperar'),
...
const DIAPER_REF=[
 {size:'RN',age:'primeiros meses',daily:'7–9/dia',monthly:'até ~220/mês',share:.06,note:'RN costuma durar pouco; evite excesso.'},
 {size:'P',age:'2–4 meses',daily:'~6/dia',monthly:'~180/mês',share:.15,note:'Ajuste pelo peso real do bebê.'},
 {size:'M',age:'5–10 meses',daily:'~5/dia',monthly:'~150/mês',share:.25,note:'Fase importante, mas não é necessariamente a mais longa.'},
 {size:'G',age:'11–20 meses',daily:'~5/dia',monthly:'~150/mês',share:.41,note:'Na referência etária da Huggies, é a fase mais longa até 2 anos.'},
 {size:'XG',age:'21–24 meses',daily:'~4/dia',monthly:'~120/mês',share:.13,note:'Pode começar antes/depois conforme peso e modelagem.'}
];
...
function renderTea(){const p=diaperPlan();$('#view-cha').innerHTML=`<div class="grid-2"><div class="card panel"><div class="section-head"><div><h2>Planejador do chá de fraldas</h2><p>Distribuição baseada na referência de consumo da Huggies, ajustada para o Ninho acompanhar até 24 meses.</p></div></div><div class="field"><label>Número estimado de convidados/presentes</label><input type="number" min="0" value="${state.showerGuests}" onchange="setGuests(this.value)"></div><div class="diaper-plan diaper-plan-5" style="margin-top:16px">${DIAPER_REF.map(r=>`<div class="diaper"><span>Tamanho</span><b>${r.size}</b><span>${p[r.size]} pacotes/presentes</span><small>${r.age} · ${r.daily}</small></div>`).join('')}</div><div class="notice" style="margin-top:14px"><b>Importante:</b> pacote não é uma unidade padrão — cada marca/tamanho traz quantidades diferentes. Use esta divisão para orientar o chá e, depois, ajuste pelo peso, adaptação à marca e ritmo real de trocas.</div></div><div class="card panel"><div class="section-head"><div><h2>Consumo de referência</h2><p>Huggies Brasil · estimativa, não regra.</p></div></div><div class="diaper-reference">${DIAPER_REF.map(r=>`<div class="diaper-ref-row"><b>${r.size}</b><div><span>${r.age}</span><small>${r.monthly} · ${r.note}</small></div></div>`).join('')}</div><div class="notice" style="margin-top:14px">A própria Huggies ressalta que tamanho e consumo variam. No inventário do Enxoval, as metas de RN/P/M/G/XG agora usam exatamente a mesma distribuição desta tela.</div><a class="btn soft link-btn" target="_blank" href="https://www.huggies.com.br/artigos/bebe/3-6-meses-do-bebe/quantas-fraldas-um-bebe-usa-por-dia">Abrir referência Huggies</a></div></div>`}
